...
"""NVFP4-KIVI software KV-cache attention backend.
...
class.  This module supplies only the E2M1 (NVFP4) codec callables.

See ``triton_nvfp4_kivi.py`` for the quantisation details and the int4-vs-nvfp4 A/B.
"""
...
Nvfp4KiviMetadata = KiviMetadata
Nvfp4KiviMetadataBuilder = KiviMetadataBuilder
...
class Nvfp4KiviAttentionBackend(AttentionBackend):
    ...
    def get_kv_cache_shape(
        ...
        # NVFP4-KIVI has the same byte budget as INT4-KIVI.
...
class Nvfp4KiviAttentionImpl(KiviAttentionImplBase):
    ...
    def _gather_dequant(self, kv_cache, block_table, seq_lens, head_size, num_kv_heads, max_seq):
        return nvfp4_kivi_gather_dequant(kv_cache, block_table, seq_lens, head_size, num_kv_heads, max_seq)

    def _fused_decode_kernel(self, q, kv_cache, block_table, seq_lens, scale):
        return nvfp4_kivi_paged_decode(q, kv_cache, block_table, seq_lens, scale)
